...
async def get_current_user_info(current_user:Annotated[User,Depends(get_current_user)]):
    """Get the currently authenticated user's information. Built explicitly
    (not a bare `return current_user`) so `tenant_name` reads from the
    `.tenant` relationship `get_current_user`'s query eager-loads — a tenant
    user has no other way to learn their own org's name, since
    `GET /tenants/{id}` is superuser-only."""
    return UserResponse(
        id=current_user.id,
